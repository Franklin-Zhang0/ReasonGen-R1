...
available_models["image_only_grpo_8_rollout_bs32_mini16_cfg_1.0_cot_no_kl_lr_5e-6_3_ds_sft2_batch_subsample_adaptive_entropy_max_coeff_3e-3_seperate_target_text_2.0_img_6.5_225"]={
    "model_path":"/blob/franklin/ckpt/image_rl/verl_janus_test/image_only_grpo_8_rollout_bs32_mini16_cfg_1.0_cot_no_kl_lr_5e-6_3_ds_sft2_batch_subsample_adaptive_entropy_max_coeff_3e-3_seperate_target_text_2.0_img_6.5/global_step_225/actor/huggingface",
    "use_cot": True,
}
...
def generate_from_geneval_jsonl(
    ...
    for idx, line in tqdm(zip(ids, lines), total=len(lines)):
        ...
        if use_two_stage:
            ...
            img_sft_format = None
        this_out_png_path_1 = os.path.join(out_dir,f"{idx:06d}_0.png")
        this_out_png_path_2 = os.path.join(out_dir,f"{idx:06d}_1.png")
        this_out_cot_path_1 = os.path.join(out_dir,f"{idx:06d}_0.txt")
        this_out_cot_path_2 = os.path.join(out_dir,f"{idx:06d}_1.txt")
        # if exist this image, skip
        if os.path.exists(this_out_png_path_1) and os.path.exists(this_out_png_path_2) and os.path.exists(this_out_cot_path_1) and os.path.exists(this_out_cot_path_2):
            continue

        if cot:
            cots, visual_img=inference_from_prompt_cot(prompt, img_sft_format)
            for i in range(len(cots)):
                with open(os.path.join(out_dir, f"{idx:06d}_{i}.txt"), "w") as f:
                    f.write(cots[i])
        else:
            visual_img=inference_from_prompt(prompt)

        PIL.Image.fromarray(visual_img[0]).save(this_out_png_path_1)
        PIL.Image.fromarray(visual_img[1]).save(this_out_png_path_2)
