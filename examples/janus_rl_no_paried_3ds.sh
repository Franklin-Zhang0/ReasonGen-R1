...
RUN_NAME="image_only_grpo_8_rollout_bs32_mini16_cfg_1.0_cot_no_kl_lr_5e-6_3_ds_train_ds"
...
TEMPLATE='A photo of {}. Detailed prompt for image generation: '
...
python3 -m verl.trainer.image_generation_rl \
    ...
    data.train_files="[/blob/franklin/datasets/Janus_RL/geneval/prompts/generation_prompts_0504_train.txt,/blob/franklin/datasets/Janus_RL/dpg_prompts/train_dpg.txt,/blob/franklin/datasets/Janus_RL/t2i_prompts/merged_t2i_train.txt]" \
    data.val_files="[/blob/franklin/datasets/Janus_RL/geneval/prompts/generation_prompts_eval.txt,/blob/franklin/datasets/Janus_RL/dpg_prompts/merged_dpg.txt,/blob/franklin/datasets/Janus_RL/t2i_prompts/merged_t2i_val.txt]" \
    ...
    img_saving.num=16 \
    reward_model.rollout.prompt_length=3076 \
    reward_model.rollout.max_num_batched_tokens=16384
    
bash ~/run_on_all_nodes.sh "tmux new-session -d -s think 'conda activate ptca; python /blob/thinking.py'"
# bash ~/run_on_all_nodes.sh "tmux new-session -d -s think 'conda activate ptca; python /blob/thinking_small.py'"
