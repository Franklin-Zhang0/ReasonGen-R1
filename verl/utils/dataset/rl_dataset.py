...
class DummyJanusDPO_PairWise_RLHFDataset(Dataset):
    """
    We assume the dataset contains a column that contains prompts and other information
    """
    def __init__(self,
    ...
    def __getitem__(self, item):
        ...
        # expand image token
        from PIL import Image
        row_dict['multi_modal_data'] = {f'Good': [Image.fromarray(np.random.rand(3, 384, 384))], 'Bad': [Image.fromarray(np.random.rand(3, 384, 384))]}
        image_inputs = {self.processor.image_processor(row_dict['multi_modal_data'][key], return_tensors='pt') for key in ['Good', 'Bad']}
        
        row_dict['multi_modal_output'] = {key: val for key, val in image_inputs.items()}
...
class JanusDPORLHFDataset(Dataset):
    """
    We assume the dataset contains a column that contains prompts and other information
    """
    ...
    def __init__(self,
        ...
        # default not store
        self.serialize_dataset = False
        self.dataframe = InterleaveDataset(parquet_files)
    ...
    def __getitem__(self, item):
        ...
        prompt_with_chat_template = self.processor.apply_sft_template_for_multi_turn_prompts(
            conversations=chat,
            sft_format=self.processor.sft_format,
            system_prompt=self.system_prompt,
        )
        ...
        is_multi_modal = f'{self.image_key}_0' in row_dict
        assert is_multi_modal, "JanusRLHFDataset only supports multimodal data"
        # expand image token
        row_dict['multi_modal_data'] = {f'image_{i}': [process_image(f'{row_dict.pop(self.image_key)}_{i}')] for i in range(2)}
        image_inputs = self.processor.image_processor(row_dict['multi_modal_data']['image'], return_tensors='pt')
